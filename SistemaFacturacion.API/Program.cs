...
// Habilitar CORS para comunicación con Blazor y MAUI
...
// Recrear base de datos si falta alguna tabla
using (var scope = app.Services.CreateScope())
{
    var db = scope.ServiceProvider.GetRequiredService<FacturacionDbContext>();
    try
    {
        // Verificar si las nuevas columnas (CajaSesionId, SaldoPendiente, etc.) existen en la BD SQLite existente
        _ = db.Facturas.Select(f => new { f.UsuarioId, f.CajaSesionId, f.PagaCon }).FirstOrDefault();
    }
    catch
    {
        // Si falta alguna columna nueva, recrear la BD con el nuevo esquema completo
...
if (app.Environment.IsDevelopment() || true)
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "Sistema Facturación API v1");
